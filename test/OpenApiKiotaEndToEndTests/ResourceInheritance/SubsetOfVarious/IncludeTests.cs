    ...
    : IClassFixture<IntegrationTestContext<OpenApiStartup<ResourceInheritanceDbContext>, ResourceInheritanceDbContext>>, IDisposable
{
    ...
    public async Task Can_include_in_primary_resources()
    {
        ...
        using (_requestAdapterFactory.WithQueryString(queryString))
        {
            ...
            response.Included.OfType<DataInFamilyHomeResponse>().Should().ContainSingle(familyHomeData => familyHomeData.Id == familyHome.StringId).Subject
                .With(familyHomeData =>
                {
            ...
            response.Included.OfType<DataInMansionResponse>().Should().ContainSingle(mansionData => mansionData.Id == mansion.StringId).Subject.With(
                mansionData =>
                {
